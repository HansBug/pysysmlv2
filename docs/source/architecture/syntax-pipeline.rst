...
The listener's exported ``RawElement`` lossless compatibility node is
separately audited in ``docs/research/raw_element_compatibility_ledger.json``.
It is not a semantic-model node and is limited to deferred non-core
productions and parser-recovery fragments; valid state, expression, action,
transition, import, alias, filter, connection, and interface paths must
produce typed source-AST nodes.
