...
Every concrete AST node declares its own readable fields and implements ``to_sysml`` and ``__str__`` by composing those fields. ``ASTNode`` only stores ``source_path`` and ``span``; it does not provide a generic child list or renderer. Model-owned documentation and comments are AST content and therefore survive export. Whitespace and non-model trivia are intentionally outside the AST contract and are handled by the formatter layer.
