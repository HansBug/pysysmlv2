...
- The public source AST is not the OMG abstract semantic model. It retains concrete-syntax choices and source provenance; the future semantic layer maps those nodes to normative SysML/KerML XMI/JSON metaclasses, resolved references, derived properties, and validation rules.
- `ASTNode` is a minimal dataclass containing only `source_path` and `span`. It must not acquire `children`, generic traversal, generic rendering, token bags, reflection helpers, or semantic identity.
...
- Keep grammar-required children as required dataclass constructor arguments. Use `Optional[...]` and defaults only for grammar alternatives that are actually optional. A node may omit `source_path` and `span` when constructed directly, but that convenience must never make a required grammar child optional.
